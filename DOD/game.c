...
AGE_RESULT game_reserve_memory_for_asteroids_bullets ()
{
    ...
    game_asteroids_ids = (actor_id*)utils_calloc (game_current_max_asteroid_count, sizeof (actor_id));
...
AGE_RESULT game_init (const HINSTANCE h_instance, const HWND h_wnd)
{
    ...
    age_result = graphics_init (game_current_max_asteroid_count, game_asteroid_live_count, game_current_max_bullet_count, game_bullet_live_count);
    if (age_result != AGE_SUCCESS)
    {
        goto exit;
    }
...
AGE_RESULT game_asteroid_add (void)
{
    ...
    if (game_asteroid_live_count == game_current_max_asteroid_count)
    {
        game_current_max_asteroid_count += game_ASTEROID_BATCH_SIZE;
...
AGE_RESULT game_asteroid_remove (void)
{
    AGE_RESULT age_result = AGE_SUCCESS;
...
AGE_RESULT game_bullet_add (void)
{
    ...
    if (game_bullet_live_count == game_current_max_bullet_count)
    {
        game_current_max_bullet_count += game_BULLET_BATCH_SIZE;
...
AGE_RESULT game_update (size_t delta_msecs)
{
    ...
    age_result = graphics_update_transforms_buffer_data (&game_player_transform_outputs, game_asteroids_transform_outputs, game_asteroid_live_count, game_bullets_transform_outputs, game_bullet_live_count);
    if (age_result != AGE_SUCCESS)
    {
        goto exit;
    }
